...
public class TranslationDictionary {
    ...
    public void register(String original, String translated) {
        ...
        //
        // Importante: entradas onde original == traduzido tambem precisam ser
        // registradas. Elas significam "manter exatamente assim" no dicionario
        // manual/cache editado pelo usuario. O fluxo antigo descartava esses
        // pares por considerar que nao havia traducao real; com isso termos como
        // nomes de mods, maquinas ou encantamentos em ingles caiam na API e eram
        // traduzidos mesmo estando no dicionario.
        if (exactMap.size() < MAX_ENTRIES) {
            exactMap.put(normalizeExactKey(origClean), transClean);
        }

        if (origClean.equalsIgnoreCase(transClean)) return; // trava exata, sem tokens
    ...
    public String lookupExact(String text) {
        ...
        String direct = exactMap.get(normalizeExactKey(text));
        if (direct != null) return direct;

        String stripped = TextFormatUtils.stripFormatting(text);
        if (stripped == null || stripped.equals(text)) return null;
        return exactMap.get(normalizeExactKey(stripped));
    }
    ...
    private String normalizeExactKey(String text) {
