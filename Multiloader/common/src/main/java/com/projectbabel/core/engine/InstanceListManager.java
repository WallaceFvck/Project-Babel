            ...
            if (resp.statusCode() == 200) {
                ...
                if (json.has("lingva")) {
                    ...
                    if (!instances.isEmpty()) {
                        ...
                        ProjectBabelCommon.LOGGER.info("[projectbabel] {} instâncias Lingva carregadas do Gist.", instances.size());
                        return;
                    }
                }
            }
        } catch (Exception e) {
            ProjectBabelCommon.LOGGER.debug("[projectbabel] Gist fetch: {}", e.getMessage());
        }
        if (cached == null) {
            cached    = HARDCODED;
            lastFetch = System.currentTimeMillis();
