...
public final class TranslationTriageManager {
    ...
    public Component triageComponentTree(Component original) {
        ...
        if (original.getContents() instanceof LiteralContents literal && original.getSiblings().isEmpty()) {
            ...
            scheduleLiteralComponent(componentKey, original, translated);
            return original;
        }
        ...
        if (text.length() < AutoTranslateConfig.getMinTextLength()) return original;
    ...
    public Component triageComponentTreeCacheOnly(Component original) {
        ...
        if (original.getContents() instanceof LiteralContents literal) {
            ...
            if (!original.getSiblings().isEmpty()) {
                ...
                String fullContextKey = buildContextKey(cacheLanguage(), LITERAL_NAMESPACE, fullText);
                String fullCachedText = lookupCachedText(fullContextKey, fullText);
                if (fullCachedText != null && !fullCachedText.equals(fullText)) {
                    Component rebuiltFull = literalLike(original, fullCachedText);
                    cacheComponent(componentContextKey(fullContextKey, original), rebuiltFull);
                    TranslationSkipRegistry.skip(rebuiltFull);
                    return rebuiltFull;
                }
            }
            ...
            String contextKey = buildContextKey(cacheLanguage(), LITERAL_NAMESPACE, text);
            String cachedText = lookupCachedText(contextKey, text);
            boolean textChanged = cachedText != null && !cachedText.equals(text);

            Component[] translatedSiblings = translateSiblingsCacheOnly(original);
            if (!textChanged && translatedSiblings == null) return original;

            MutableComponent rebuilt = Component.literal(textChanged ? cachedText : text)
                .withStyle(original.getStyle() == null ? Style.EMPTY : original.getStyle());
            appendSiblings(rebuilt, translatedSiblings, original);
            return rebuilt;
        }
    ...
    public String triageStringCacheOnly(String text) {
        ...
        if (text.length() < AutoTranslateConfig.getMinTextLength()) return text;
        ...
        scheduleCacheOnlyText(contextKey, text, TextFilter.isScreenFilterBypassed());
        return text;
    }
    ...
    public void warmText(String originalText, String translatedText) {
        ...
        if (translatedText.equals(originalText) && needsTranslation(originalText)) return;
    ...
    private void scheduleText(String contextKey, String originalText, boolean nativeLookup, boolean bypassScreenFilter) {
        ...
        CompletableFuture
            ...
            .exceptionally(error -> {
                ProjectBabelMod.LOGGER.debug("[projectbabel] Triage text failed: {}", error.toString());
                return null;
            })
            .whenComplete((ignored, error) -> pending.remove(contextKey));
    }

    private void scheduleCacheOnlyText(String contextKey, String originalText, boolean bypassScreenFilter) {
        if (contextKey == null || originalText == null) return;
        if (textCache.containsKey(contextKey)) return;

        String pendingKey = "cache:" + contextKey;
        if (pending.putIfAbsent(pendingKey, Boolean.TRUE) != null) return;

        long taskGeneration = generation.get();
        CompletableFuture
            .supplyAsync(() -> resolveTextFromCacheOnly(originalText, bypassScreenFilter), activeExecutor())
            .thenAccept(translated -> {
                if (translated != null && isCurrentGeneration(taskGeneration)) {
                    cacheText(contextKey, sanitizeCachedText(originalText, translated));
                }
            ...
            .exceptionally(error -> {
                ProjectBabelMod.LOGGER.debug("[projectbabel] Triage cache-only failed: {}", error.toString());
                return null;
            })
    ...
    private void scheduleComponent(String componentKey, String textContextKey, Component original, boolean cacheTextResult) {
        ...
        CompletableFuture
            ...
            .exceptionally(error -> {
                ProjectBabelMod.LOGGER.debug("[projectbabel] Triage component failed: {}", error.toString());
                return null;
            })
            .whenComplete((ignored, error) -> pending.remove(pendingKey));
    }
    ...
    private String resolveTextInternal(String text, boolean nativeLookup) {
        ...
        String universal = UniversalTermsDictionary.getInstance().lookupExact(plain, AutoTranslateConfig.getSourceLang(), target);
        ...
        if (nativeLookup) {
            cached = manager.getCachedTranslation(plain, AutoTranslateConfig.getSourceLang(), target);
            if (cached == null || cached.isBlank()) {
    ...
    private String resolveTextFromCacheOnly(String text, boolean bypassScreenFilter) {
        if (bypassScreenFilter) {
            try (TextFilter.ScreenFilterBypass ignored = TextFilter.bypassScreenFilter()) {
                return resolveTextFromCacheOnlyInternal(text);
            }
        }
        return resolveTextFromCacheOnlyInternal(text);
    }

    private String resolveTextFromCacheOnlyInternal(String text) {
        if (text == null || text.isBlank()) return text;
        ...
        cached = manager.getCachedTranslation(text, source, target);
        return cached == null ? null : TextFormatUtils.preserveEdgeWhitespace(text, cached);
    }
    ...
    private String lookupCachedText(String contextKey, String text) {
        ...
        String external = TranslationManager.getInstance()
            .getCachedTranslation(text, AutoTranslateConfig.getSourceLang(), apiLanguage());
    ...
    private boolean canRun() {
        return AutoTranslateConfig.isEnabled() && LanguageDetector.shouldModBeActive();
    }
    ...
    private void refreshLanguageCache() {
        ...
        String configuredTarget = AutoTranslateConfig.getTargetLang();
        if (configuredTarget == null) configuredTarget = "";
        boolean followClientLanguage = AutoTranslateConfig.isFollowClientLanguage();
    ...
    private String currentLanguageCode() {
        ...
        return normalizeClientCode(AutoTranslateConfig.getTargetLang());
    }
    ...
    private static boolean shouldPreferExplicitCache(String key) {
        return key != null && !isVanillaTranslationKey(key);
    }
    ...
    private boolean computeNativeTranslationValidity(String key) {
        try {
            ...
            if (isVanillaTranslationKey(key)) return true;
