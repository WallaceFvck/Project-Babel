...
import com.projectbabel.translation.engine.GoogleTranslateEngine;
import com.projectbabel.translation.engine.LingvaEngine;
...
public class TranslationManager {
    ...
    private TranslationManager() {
        ...
        int workerCount = Math.max(preloadConcurrencyLimit(), Math.max(normalConcurrencyLimit(), turboConcurrencyLimit()));
        this.executor = Executors.newFixedThreadPool(workerCount, r -> {
            Thread t = new Thread(r, "projectbabel-UltraWorker");
            t.setDaemon(true);
            t.setPriority(Thread.NORM_PRIORITY);
            return t;
        });
        for (int i = 0; i < workerCount; i++) executor.submit(this::workerLoop);
        ...
        ProjectBabelMod.LOGGER.info(
            ...
            workerCount,
            AutoTranslateConfig.isTurboMode(),
    ...
    private int effectiveWorkerCount() {
        return AutoTranslateConfig.getMaxConcurrentRequests();
    }

    private int normalConcurrencyLimit() {
        return Math.max(1, Math.min(32, effectiveWorkerCount()));
    }

    private int turboConcurrencyLimit() {
        return Math.max(1, Math.min(512, effectiveWorkerCount()));
    }

    private int preloadConcurrencyLimit() {
        return Math.max(1, Math.min(1024, effectiveWorkerCount()));
    }

    public int getActiveConcurrencyLimit() {
        if (PreloadAcceleration.isActive()) return preloadConcurrencyLimit();
        return AutoTranslateConfig.isTurboMode()
            ? turboConcurrencyLimit()
            : normalConcurrencyLimit();
    }
    ...
    public String getTranslation(String strippedText, String sourceLang, String targetLang) {
        if (!AutoTranslateConfig.isEnabled() || shuttingDown.get()) return null;
        ...
        // 3. Anti-loop: texto já é output de uma tradução anterior
        if (cache.isAlreadyTranslated(strippedText)) return null;

        // 4. Lookup exato no dicionário aprendido (sem API)
    ...
    private String getTranslationBlockingInternal(
        ...
        if (!AutoTranslateConfig.isEnabled() || shuttingDown.get()) return strippedText;
        if (!TextFilter.isScreenFilterBypassed()) {
        ...
        try {
            String result = selectEngine(text, src, tgt)
                .get(AutoTranslateConfig.getRequestTimeoutMs() + 3000L, TimeUnit.MILLISECONDS);
            ...
            ProjectBabelMod.LOGGER.debug("[projectbabel] Falha sincrona '{}': {}", text, e.getMessage());
            return text;
        }
    }
    ...
    private void workerLoop() {
        while (!shuttingDown.get() && !Thread.currentThread().isInterrupted()) {
            try {
                ...
                finally { activeTranslations.decrementAndGet(); }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt(); break;
            } catch (Exception e) {
                ProjectBabelMod.LOGGER.debug("[projectbabel] Worker: {}", e.getMessage());
            }
        }
    }

    private void acquireDynamicSlot() throws InterruptedException {
        while (!shuttingDown.get() && !Thread.currentThread().isInterrupted()) {
            int active = activeTranslations.get();
            if (active < getActiveConcurrencyLimit()
                && activeTranslations.compareAndSet(active, active + 1)) {
                return;
            }
            Thread.sleep(10L);
        }
        throw new InterruptedException("Translation manager shutting down");
    }
    ...
    private void executeTask(TranslationTask task) {
        try {
            ...
            if (hint != null) {
                ProjectBabelMod.LOGGER.debug("[projectbabel] Hint '{}': {}", task.text, hint);
            }
            ...
            if (result != null && !result.isBlank()) {
                ...
                ProjectBabelMod.LOGGER.debug("[projectbabel] [p{}] '{}' -> '{}'",
                    task.priority, task.text, result);
            }

        } catch (TimeoutException e) {
            ProjectBabelMod.LOGGER.debug("[projectbabel] Timeout: {}", task.text);
            failedKeys.put(task.pendingKey, System.currentTimeMillis() + FAILED_KEY_TTL_MS);
        } catch (Exception e) {
            ProjectBabelMod.LOGGER.debug("[projectbabel] Falha '{}': {}", task.text, e.getMessage());
    ...
    private CompletableFuture<String> selectEngine(String text, String src, String tgt) {
        ...
        if (lingvaActiveSince > 0
            ...
            ProjectBabelMod.LOGGER.info("[projectbabel] Retornando ao Google após cooldown.");
        }
    ...
    public void resetEngineFallback() {
        ...
        failedKeys.clear();
        ProjectBabelMod.LOGGER.info("[projectbabel] Fallback resetado — Google voltará a ser usado.");
    }
    ...
    public int getActiveTranslationCount() { return activeTranslations.get(); }
    ...
    public void shutdown() {
        if (shuttingDown.compareAndSet(false, true)) {
            ...
            ProjectBabelMod.LOGGER.info("[projectbabel] Desligado.");
        }
    }
}
